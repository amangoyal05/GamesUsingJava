    	...
    	public Game () {
        	...
        	while(!gameOver) { 									//runs until the game is over
            		...
            		if(gb.getStartTile() != null && gb.getEndTile() != null && gb.getStartTile().getPiece() != null) {
                		if(playerTwo.isTurn() && gb.getStartTile().getPiece().getPieceAlliance() == Alliance.WHITE || playerOne.isTurn() && gb.getStartTile().getPiece().getPieceAlliance() == Alliance.BLACK) {
                    			if(gb.getFirstClick()) {
        	...
        	if(gb.getStartTile().isOccupied() && gb.getStartTile().getPiece().isValidMove(gb.getStartTile(), gb.getEndTile())) {
            		if(gb.getStartTile().getPiece().isValidPath(gb.getStartTile(), gb.getEndTile(), gb.getTileMap())) {
                		...
                		if(gb.getEndTile().getPiece().getPieceType() == PieceType.PAWN) {
                    			...
                    			if((gb.getEndTile().getPawn().getPieceAlliance() == Alliance.WHITE && gb.getEndTile().getRow() == 0) ||(gb.getEndTile().getPawn().getPieceAlliance() == Alliance.BLACK && gb.getEndTile().getRow() == 7)) {
                        			pawnPromotion(gb);
                    			}
                		}
